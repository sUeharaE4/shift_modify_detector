...
def dftregistration(buf1ft, buf2ft, usfac):
    ...
    if usfac == 0:
        ...
        if usfac > 2:
            ...
            CC = np.conjugate(dftups(buf2ft*np.conjugate(buf1ft),
                                     np.ceil(usfac*1.5), np.ceil(usfac*1.5),
...
def FTpad(imFT, outsize):
    ...
    imFTout[slice(cenout_cen[0], cenout_cen[0]+Nin[0]),
            slice(cenout_cen[1], cenout_cen[1]+Nin[1])] = imFT
